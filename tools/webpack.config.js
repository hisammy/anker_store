...
const webpackConfig = {
  output: {
    publicPath: process.env.NODE_ENV === 'production' ? 'https://d30w2ife1a7e8h.cloudfront.net/' : '/',
    sourcePrefix: '  '
  },
...
/*//
// Configuration for the deals bundle (/deals/index.js)
// -----------------------------------------------------------------------------

const dealsConfig = merge({}, webpackConfig, {
  entry: {
    app: [
  ...
  output: {
    path: path.join(__dirname, '../build/deals'),
...
const appConfig = merge({}, webpackConfig, {
  entry: {
    ...
    //powerHouse:['./deals/public/javascript/powerHouse.js']
  },
  output: {
...
const serverConfig = merge({}, webpackConfig, {
  ...
  target: 'node',
  externals: [
    /^\.\/assets\.json$/, function filter(context, request, cb) {
