    ...
    on roles fetch(:npm_roles) do
      within fetch(:npm_target_path, release_path) do
        with fetch(:npm_env_variables, {}) do
          execute :mkdir,"-p","#{fetch(:shared_path)}/node_modules"
          execute :cp,"-r","#{fetch(:shared_path)}/node_modules","."
          #execute :cp,"#{fetch(:shared_path)}/src/newrelic.js","src/"                    
          execute :npm, 'install' , fetch(:npm_flags)
          execute :cp,"-r","node_modules","#{fetch(:shared_path)}/"
